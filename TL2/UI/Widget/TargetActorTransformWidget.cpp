...
void UTargetActorTransformWidget::RenderWidget()
{
	...
	if (SelectedActor)
	{
		...
		static TArray<TPair<FString, UClass*>> AddableComponentTypes;
		...
		if (!bComponentTypesInitialized)
		{
			...
			for (UClass* Class : DerivedClasses)
			{
				...
				{
					AddableComponentTypes.push_back({ ClassName, Class });
				}
			}

			// 이름순 정렬
			std::sort(AddableComponentTypes.begin(), AddableComponentTypes.end(),
