...
void URenderer::RenderScene(UWorld* World, ACameraActor* Camera, FViewport* Viewport)
{
	...
	CurrentViewMode = Viewport->GetViewportClient()->GetViewModeIndex();
	// 광원 정보 상수버퍼 구조체 내용 갱신
	UpdateLightingBuffer(World, Camera);
...
void URenderer::UpdateLightingBuffer(UWorld* InWorld, ACameraActor* InCameraActor)
{
	...
	ZeroMemory(&LightingCBufferData, sizeof(FLightingBufferType));
	ULevel* CurrentLevel = InWorld->GetLevel();
	if (UAmbientLightComponent* AmbientLightComponent = CurrentLevel->GetComponent<UAmbientLightComponent>())
	{
		if (AmbientLightComponent->GetVisible())
		{
			const FColor& Color = AmbientLightComponent->GetLightColor();
			const float Intensity = AmbientLightComponent->GetIntensity();
			LightingCBufferData.Ambient.Color = FVector4(Color.R / 255.0f, Color.G / 255.0f, Color.B / 255.0f, 1.0f);
			LightingCBufferData.Ambient.Intensity = Intensity;
		}
	}
	if (UDirectionalLightComponent* DirectionalLightComponent = CurrentLevel->GetComponent<UDirectionalLightComponent>())
	{
		if (DirectionalLightComponent->GetVisible())
		{
